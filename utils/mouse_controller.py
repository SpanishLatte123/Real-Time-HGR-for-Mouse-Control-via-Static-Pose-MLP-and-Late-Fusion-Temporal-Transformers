...
class HgrMouseControl:
    ...
    def sensitivity(self):
        ...
        range_ = HgrMouseControl.sensitivity_range
    ...
    def margin(self):
        ...
        max_margin = HgrMouseControl.max_detection_margin
        min_margin = HgrMouseControl.min_detection_margin
    ...
    def get_palm_center(self, landmark_vector):
        self.frame += 1
    ...
    def handle_pointer(self, hand_center, hand_pose):
        ...
        if hand_pose == HgrMouseControl.Event.MOVE or hand_pose == HgrMouseControl.Event.LEFT_DOWN:
    ...
    def _on_pose_change(self, new_hand_pose):
        if self.current_mouse_state == HgrMouseControl.MouseState.STANDARD:
            self._on_pose_change_standard(new_hand_pose)
        if self.current_mouse_state == HgrMouseControl.MouseState.SCROLLING:
            self._on_pause_change_other(new_hand_pose, HgrMouseControl.Event.SCROLL, HgrMouseControl.MouseState.STANDARD)
        if self.current_mouse_state == HgrMouseControl.MouseState.DRAGGING:
            self._on_pause_change_other(new_hand_pose, HgrMouseControl.Event.LEFT_DOWN, HgrMouseControl.MouseState.STANDARD)
    ...
    def _on_pose_change_standard(self, new_hand_pose):
        if new_hand_pose == self.Event.MOVE:
            ...
            self.enter_state(HgrMouseControl.MouseState.DRAGGING)
        elif new_hand_pose == self.Event.SCROLL:
            self.enter_state(HgrMouseControl.MouseState.SCROLLING)
    ...
    def operate_mouse(self, hand, palm_center, confidence, min_confidence=0.5):
        ...
        if self.current_mouse_state == HgrMouseControl.MouseState.STANDARD:
            ...
            if hand.pose == HgrMouseControl.Event.MOVE or hand.pose == HgrMouseControl.Event.STOP:
        ...
        elif self.current_mouse_state == HgrMouseControl.MouseState.SCROLLING:

            # Scroll up or down
            if hand.pose == HgrMouseControl.Event.SCROLL:
        ...
        elif self.current_mouse_state == HgrMouseControl.MouseState.DRAGGING:
